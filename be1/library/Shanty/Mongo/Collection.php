...
abstract class Shanty_Mongo_Collection
{
	...
	public static function one(array $query = array(), array $fields = array())
	{
        ...
        if(isset($GLOBALS["profiling"])) { $callers=debug_backtrace(); $GLOBALS["profiling"]->mark("MONGO one() before find",get_called_class(),$callers[1]['function'],static::getCollectionName(),print_r($query,1)); }

		$data = static::getMongoCollection(false)->findOne($query, $fields);
        if(isset($GLOBALS["profiling"])) { $callers=debug_backtrace(); $GLOBALS["profiling"]->mark("MONGO one() after find",get_called_class(),$callers[1]['function'],static::getCollectionName(),print_r($query,1)); }
	 ...
	 */
	public static function all(array $query = array(), array $fields = array(), $limit = FALSE)
	{
		$inheritance = static::getCollectionInheritance();
		if (count($inheritance) > 1) {
			$query['_type'] = $inheritance[0];
        ...
        if(isset($GLOBALS["profiling"])) { $callers=debug_backtrace(); $GLOBALS["profiling"]->mark("MONGO all() before find",get_called_class(),$callers[1]['function'],static::getCollectionName(),print_r($query,1)); }


        $cursor = ($limit !== FALSE) ? static::getMongoCollection(false)->find($query, $fields)->limit($limit) : static::getMongoCollection(false)->find($query, $fields);
        if(isset($GLOBALS["profiling"])) { $callers=debug_backtrace(); $GLOBALS["profiling"]->mark("MONGO all() after find",get_called_class(),$callers[1]['function'],static::getCollectionName(),print_r($query,1)); }
