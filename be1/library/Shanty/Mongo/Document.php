...
class Shanty_Mongo_Document extends Shanty_Mongo_Collection implements ArrayAccess, Countable, IteratorAggregate
{
	 ...
	 * @param $object
	 * @param $options
	 * Because of Mongo sharding we set multiple parameter to be always true
	 */
    ...
    */
		...
		if (!$skipRequired) {
			...
			foreach ($requiredProperties as $property) {
				if (!isset($exportData[$property]) || (is_array($exportData[$property]) && empty($exportData[$property]))) {
					require_once 'Shanty/Mongo/Exception.php';
					throw new Shanty_Mongo_Exception("Property '{$property}' must not be null, for collection ".$this->_config["collection"]);
				}
			}
	...
	public function save($entierDocument = false, $safe = true)
	{
		...
		else
		{
            ...
            if(isset($newversion['errmsg'])) error_log("Mongo save() error: ".$newversion['errmsg']);
