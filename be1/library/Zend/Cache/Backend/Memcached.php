...
class Zend_Cache_Backend_Memcached extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface
{
    ...
    public function __construct(array $options = array())
    {
        ...
        $server_counter = 0; $available_server = 0;
        if($this->_options['random_server_access']) shuffle($this->_options['servers']);
        foreach ($this->_options['servers'] as $server) {
            ...
            if(!$this->testServer($server)) continue; //checks if memcache service is reachable on server:port
            $available_server++;
        ...
        if($available_server==0) error_log("Memcached error: no Memcached servers are available!!!");
        elseif($server_counter>$available_server) error_log("Memcached warning: $available_server out of $server_counter Memcached servers are available");
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        foreach ($this->_options['servers'] as $server) {
            ...
            $memcache_obj->connect($server['host'], $server['port']);
            $result = $memcache_obj->set($id, array($data, time(), $lifetime), $flag, $lifetime);
            $memcache_obj->close();
        }
    ...
    protected function testServer($server) {
        if(!isset($server["host"])) return false;

        $fp = @fsockopen($server["host"], $server["port"], $errno, $errstr, $server["timeout"]);
        if (!$fp) {
            return false;
        } else {
            fclose($fp);
