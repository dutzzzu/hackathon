...
abstract class Zend_Controller_Action implements Zend_Controller_Action_Interface
{
     ...
     * @return void
     */
    protected function _redirect($url, array $options = array(), $keep_query_string_params=true)
    {
        if($keep_query_string_params) $url = $this->addQueryStringParams($url);
        $this->_helper->redirector->gotoUrl($url, $options);
    }

    protected function addQueryStringParams($url) {
        $prefix = "?";
        if(strpos($url,"?")!==false) $prefix = "&";

        $query_string = !empty($_SERVER['QUERY_STRING'])?$prefix.$_SERVER['QUERY_STRING']:"";
